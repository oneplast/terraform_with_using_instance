...
sudo mkdir -p /infra_practice/terraform/npm_1/volumes/data
sudo mkdir -p /infra_practice/terraform/npm_1/volumes/etc/letsencrypt
sudo mkdir -p /infra_practice/terraform/mysql_1/volumes/var/lib/mysql
sudo mkdir -p /infra_practice/terraform/mysql_1/volumes/etc/mysql/conf.d

# 도커 네트워크 생성
sudo docker network inspect common >/dev/null 2>&1 || sudo docker network create common
...
docker run -d \
  ...
  -e TZ=Asia/Seoul \
  -v /infra_practice/terraform/npm_1/volumes/data:/data \
  -v /infra_practice/terraform/npm_1/volumes/etc/letsencrypt:/etc/letsencrypt \
...
docker run -d \
  --name mysql_1 \
  --restart unless-stopped \
  -v /infra_practice/terraform/mysql_1/volumes/var/lib/mysql:/var/lib/mysql \
...
GRANT ALL PRIVILEGES ON *.* TO 'oneplast'@'%';
